...
class SAC(nn.Module):
    ...
    def select_action(self, s: Tensor, prob: bool = False):
        ...
        log_det_jac = torch.sum( torch.log( torch.abs( (1.0 - a.pow(2))/2.0 * delta ) ), dim = 1, keepdim = True)
    ...
    def update(self):
        ...
        with torch.no_grad():
            ...
            # mu, logvar = self.policy(s_).chunk(2, dim = 1)
            # std = torch.exp(0.5 * logvar)
            # eps = torch.randn_like(std)
            # u_ = mu + std * eps ## unbounded u_ (-inf, inf)
            # a_ = torch.tanh(u_)
            a_, log_prob = self.select_action(s_, prob = True)

        #     ## compute the probability of the action under a standard normal distribution
        #     next_a_unbounded_log_prob = -0.5 * (logvar + torch.log(torch.tensor(2 * torch.pi))).sum(dim = 1, keepdim = True)
        #     next_a_unbounded_log_prob = next_a_unbounded_log_prob.unsqueeze(-1)

        #     next_a_bounded_log_prob = next_a_unbounded_log_prob - torch.log(1 - a_bounded_.pow(2) + 1e-6).sum(dim=-1, keepdim = True)

        #     ## compute the value of this action using the least optimistic critic
        #     s_a_ = torch.cat([s_, a_bounded_], dim = 1)
        ...
        #     y = r + (~t) * self.gamma * (next_q - self.alpha * next_a_bounded_log_prob)
        ...
        # std = torch.exp(0.5 * logvar)
        # eps = torch.randn_like(std)
        # a_unbounded = mu + std * eps
        # a_bounded = torch.tanh(a_unbounded)

        # a_unbounded_logprob = -0.5 * (logvar + torch.log(torch.tensor(2 * torch.pi))).sum(dim=1, keepdim=True)
        # a_unbounded_logprob = a_unbounded_logprob.unsqueeze(-1)
        # a_bounded_logprob = a_unbounded_logprob - torch.log(1 - a_bounded.pow(2) + 1e-6).sum(dim = -1, keepdim = True)

        # ## state action pair with fresh action sampled in the update loop
        # sa = torch.cat([s, a_bounded], dim = 1)
        # q_value = torch.minimum(self.Q1(sa), self.Q2(sa))
        # policy_loss = (self.alpha * a_bounded_logprob - q_value).mean()
        ...
        # return {'q1_loss': q1_loss.item(), 'q2_loss': q2_loss.item(), 'a_min': a.min().item(), 'a_max': a.max().item(), 'a_mean': a.mean().item()}
...
if __name__ == '__main__':
    ...
    observation = torch.randn(32, observation.shape[0], device = device)
    sac.select_action(observation, prob = True)
    ...
    # # ax[2].plot(hist['a_min'])
    # # ax[2].plot(hist['a_mean'])
    # # ax[2].plot(hist['a_max'])
    # # plt.show()